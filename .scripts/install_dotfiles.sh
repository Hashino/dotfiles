...
for app_config in */ ; do
  ...
  #remove command
  mv ${local_config}/${app_config} ${local_config}/${app_config}_old >> $log_file 2>&1
  if [ $? -eq 0 ]; then
    echof $NORMAL "old config found for: ${app_config} moved to ${app_config}_old"
  fi
...
echo " "
echof $TITLE "Importing ${QUOTE}wallpaper" 1
ln -s "${dotfiles_local}/wallpaper.png" "${HOME}" >> $log_file 2>&1
check_success

echo " "
echof $TITLE "Installing cursor theme ${QUOTE}Simp1e-Nord-Darker-Hash" 1
...
echo " "
echof $TITLE "Symlinking ${QUOTE}.local/${NORMAL}${TITLE} to home" 1
